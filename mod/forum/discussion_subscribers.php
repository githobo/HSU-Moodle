<?php  // $Id: discussion_subscribers.php,v 1.1 2008/02/26 18:55:49 arborrow Exp $

    require_once("../../config.php");
    require_once("lib.php");

    $id    = required_param('id',PARAM_INT);           // discussionid
    $group = optional_param('group',0,PARAM_INT);      // change of group
    $edit  = optional_param('edit',-1,PARAM_BOOL);     // Turn editing on and off

    if (! $discussion = get_record("forum_discussions", "id", $id)) { 
        error("Discussion ID is incorrect");
    }
    ...
    if (!has_capability('mod/forum:viewsubscribers', $context) && !has_capability('mod/forum:managesubscriptions', $context)) {
        error('You do not have the permission to view discussion subscribers');
    }

    unset($SESSION->fromdiscussion);

    add_to_log($course->id, "forum", "view discussion subscribers", "discussion_subscribers.php?id=$discussion->id", $discussion->id, $cm->id);

    $strsubscribeall = get_string("subscribeall", "forum");
    $strsubscribenone = get_string("subscribenone", "forum");
    $strsubscribers = get_string("subscribers", "forum");
    $strdiscussion = get_string("discussion", "forum");
    $strforums      = get_string("forums", "forum");

    $navigation = "<a href=\"index.php?id=$course->id\">$strforums</a> ->
       <a href=\"view.php?f=$forum->id\">".format_string($forum->name,true)."</a> -> $strdiscussion $strsubscribers";

    if (has_capability('mod/forum:managesubscriptions', $context)) {
        if ($edit != -1) {
            $USER->subscriptionsediting = $edit;
        }
        print_header_simple("$strdiscussion $strsubscribers", "", "$navigation",
            "", "", true, forum_update_discussion_subscriptions_button($course->id, $id));
    } else {
        print_header_simple("$strdiscussion $strsubscribers", "", "$navigation", "", "", true, '');
        unset($USER->subscriptionsediting);
    }

/// Check to see if groups are being used in this forum
    $groupmode = groupmode($course, $cm);
    $currentgroup = setup_and_print_groups($course, $groupmode, "discussion_subscribers.php?id=$discussion->id");

    if (empty($USER->subscriptionsediting)) {         /// Display an overview of subscribers

        if (! $users = forum_discussion_subscribed_all_users($course, $discussion, $currentgroup)) {

            print_heading(get_string("nodiscussionsubscribers", "forum"));

        } else {

            print_heading("$strdiscussion " . get_string("subscribersto","forum", "'".format_string($discussion->name)."'"));
...
/// Get all existing subscribers for this discussion or the forum.
    if (!$subscribers = forum_discussion_subscribed_all_users($course, $discussion, $currentgroup)) {
        $subscribers = array();
    }
...
/// If no search results then get potential subscribers for this forum excluding users already subscribed
    ...
    include('discussion_subscriber.html');
