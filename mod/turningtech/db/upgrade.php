...
/// First example, some fields were added to the module on 20070400
    ...
    /// Define field course to be added to turningtech
        $table = new XMLDBTable('turningtech');
        $field = new XMLDBField('course');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0', 'id');
    /// Launch add field course
        $result = $result && add_field($table, $field);

    /// Define field intro to be added to turningtech
        $table = new XMLDBTable('turningtech');
    ...
    /// Define field introformat to be added to turningtech
        $table = new XMLDBTable('turningtech');
...
/// "01" in the last two digits of the version
    ...
    /// Define field timecreated to be added to turningtech
        $table = new XMLDBTable('turningtech');
    ...
    /// Define field timemodified to be added to turningtech
        $table = new XMLDBTable('turningtech');
    ...
    /// Define index course (not unique) to be added to turningtech
        $table = new XMLDBTable('turningtech');
...
/// Third example, the next day, 20070402 (with the trailing 00), some inserts were performed, related with the module
    ...
    /// Add some actions to get them properly displayed in the logs
        $rec = new stdClass;
        $rec->module = 'turningtech';
        $rec->action = 'add';
        $rec->mtable = 'turningtech';
        $rec->filed  = 'name';
...
/// always the last line in the script
    ...
    /**
     * remove the old turningtech table
     */
    if($result && $oldversion < 2008051301) {
      $table = new XMLDBTable('turningtech');
    ...
    if($result && $oldversion < 2009052500) {
      $table = new XMLDBTable('device_mapping');
